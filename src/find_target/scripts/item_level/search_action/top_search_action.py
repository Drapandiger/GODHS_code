...
from item_level.view_pose_prediction.view_pose_prediction import top_search
...
def top_search_action(scene_name, room_name, carrier_name, target_name,
    ...
    # Step 1: 获取EE的poses
    EE_poses = top_search(carrier_area_occupancy, obstacles_area_occupancy,
                          room_x_max, room_x_min, room_y_max, room_y_min,
    ...
    for chassis_pose, ee_poses in sorted_footprint_dict.items():
        ...
        transformed_ee_poses = [
            transform_to_base_frame(list(chassis_pose), list(ee_pose)) for ee_pose in ee_poses
        ]
    ...
    try:
        ...
        depth_sub.unregister()
        semantic_sub.unregister()
        print(f"No target '{target_name}' found on top of {carrier_name}")
        save_knowledge_base_of_items(scene_name, room_name, carrier_name, target_name,
